...
// Sofia: ONE CGM feed (BGNAP, sipbg.gov.bg) split by route_type into two
// modes — buses+trolleybuses (3, 11) on the road graph and trams+metro (0, 1)
// on the rail graph. Trolleybuses ride the bus network in green per-line
// colors; the metro lines M1–M4 keep their official colors straight from
// routes.txt and trigger the engine's metro treatment (wide ribbon, station
// discs, always-on names) via the M-prefixed line keys.
// Usage: node pipeline/build.mjs [--all | lines...] [--tram all|7,M2]
...
// Line keys mix bare bus numbers with mode-prefixed ones (ТБ9, ТМ12А, M2) and
// with lettered bus lines (N1, X10). Sorting splits each key into prefix,
// number and suffix so 9 < 10 holds INSIDE every family, and the families
// themselves stay grouped: buses, then M/N/X/Y, then ТБ, then ТМ.
const keyParts = (s) => {
  const m = /^(\D*)(\d*)(.*)$/.exec(s);
...
// that OSM left as islands (see the call site for the Sofia specifics).
function weldRailGaps(elements) {
  const SERVICE_X = new Set(['yard', 'siding', 'spur', 'crossover']);
  const ways = elements.filter((e) => e.type === 'way' && railKind(e.tags) === 'subway'
...
// Sofia numbers every mode from 1 upwards, so bus 9, trolleybus 9 and tram 9
// are three different lines. Bare numbers would weld the bus and trolleybus
// networks together (both ride the road cfg) and make the line list ambiguous,
// so the non-bus street modes carry the operator's own prefixes — the ones CGM
// uses in its route ids (A…/ТБ…/ТМ…): ТБ = тролейбус, ТМ = трамвай. Buses keep
// the bare number, the metro keeps M1–M4.
// Feed quirk: suffix letters come in both alphabets ("22A" latin, "12А"
// cyrillic). The glyphs are identical, so the latin lookalikes are folded into
// cyrillic — otherwise one line can appear twice under visually equal names.
const CYR = { A: 'А', B: 'В', E: 'Е', K: 'К', M: 'М', H: 'Н', O: 'О', P: 'Р', C: 'С', X: 'Х' };
const norm = (sn) => sn.replace(/[A-Z]$/, (c) => CYR[c] || c);

// ONE feed, THREE cfgs: routeTypes splits the CGM bundle per mode (3 = bus and
...
const MODES = [{
  ...
  all: busAll, lines: busList.length ? busList : (busAll ? [] : ['76']),
  feeds: [
    {
...
// metro must NOT share a graph — a metro shape often passes closer to the tram
// tracks running above its tunnel than to the subway axis (the two share the
// whole Мария Луиза / Витоша axis), so a mixed graph lured Viterbi onto
// disconnected tram rails and broke the line. railKeep filters the OSM ways
// per cfg before building.
const tramAll = tramLines.length === 1 && tramLines[0] === 'all';
const tramSel = tramLines.filter((l) => !/^M[0-9]/.test(l));
const metroSel = tramLines.filter((l) => /^M[0-9]/.test(l));
if (tramAll || tramSel.length) MODES.push({
  mode: 'tram', label: 'trams', osmFile: 'data/osm/sofia-rail.json',
  graphMode: 'tram', railKeep: new Set(['tram']),
  color: '#d6212b', colorDark: '#7c1116',
  // `--tram 7` is accepted as a shorthand for the ТМ7 key
  all: tramAll, lines: tramAll ? [] : tramSel.map((l) => (/^ТМ/.test(l) ? l : 'ТМ' + norm(l.toUpperCase()))),
  feeds: [
    { tag: 'cgm', dir: 'data/gtfs', mapKey: (sn) => 'ТМ' + norm(sn), routeTypes: ['0'] },
  ],
});
if (tramAll || metroSel.length) MODES.push({
  mode: 'tram', label: 'metro', osmFile: 'data/osm/sofia-rail.json',
  graphMode: 'tram', railKeep: new Set(['subway']),
  color: '#d6212b', colorDark: '#7c1116',
  all: tramAll, lines: tramAll ? [] : metroSel,
  feeds: [
    { tag: 'cgm', dir: 'data/gtfs', mapKey: (sn) => sn, routeTypes: ['1'] },
  ],
});
...
async function processMode(cfg) {
  ...
  for (const feed of cfg.feeds) {
    ...
    for (const r of await readCsv(join(fdir, 'routes.txt'))) {
      ...
      if (r.route_type === '11') {
        ...
        // CGM ships the official metro colors (M1 red, M2 blue, M3 green, M4 yellow)
  ...
  // OSM often maps a metro as per-direction tunnels that meet nowhere: at
  // junctions the ways pass within metres but share NO node, which strands the
  // branches of a line on separate islands. Every dangling subway endpoint gets
  // welded to the nearest other-way vertex within 60 m so Viterbi can route
  // through — here it matters where M1/M2/M4 split off the central tunnel.
