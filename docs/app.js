...
async function init() {
  ...
  map.addControl(new maplibregl.AttributionControl({ compact: true, customAttribution: 'Timetables: CGM GTFS via BGNAP (CC BY 4.0)' }));
  ...
  const nMetro = meta.lines.filter((l) => l.mode === 'tram' && /^M[0-9]/.test(l.line)).length;
  ...
  const PALETTE = [
    ...
    // Sofia Metro official line colors (uppercase — exactly as the pipeline
    // emits them from routes.txt; a color missing here degrades to the
    // darken() fallback icon, so keep the list in sync)
    ['#EB2227', '#811315'], // M1
    ['#1C75BC', '#0F4067'], // M2
    ['#39B54A', '#1F6429'], // M3
    ['#FFDE17', '#8C7A0D'], // M4
  ...
  async function exportBBox(bbox, btn, doneLabel, opts) {
    ...
    try {
      ...
      const txt = '© OpenStreetMap contributors · OpenFreeMap · GTFS: CGM Sofia via BGNAP (CC BY 4.0)';
  ...
  // NOT fitBounds(meta.bbox): single regional lines reach deep into Ilfov, so
  // fitting the data bbox would open on countryside with the city as a small
  // core. Open on central Sofia instead.
  map.jumpTo({ center: [23.3220, 42.6930], zoom: 11.2 });
}
